-- =====================================================================
-- 0013 · A CONTRACAPA COM FOTO, DE PÁGINA INTEIRA
--
-- Instrução dela, em agosto/26: a última página — a de contato — passa
-- a ter foto de fundo ocupando a tela inteira, como a capa, "é o
-- fechamento da proposta". Antes ela era creme com o logo terracota no
-- meio, e ao lado de uma capa de foto cheia parecia que o documento
-- tinha acabado antes da hora.
--
-- A foto mora na PROPOSTA, e não só no padrão da casa, pelo mesmo
-- motivo do about_img: trocar a foto de fechamento não pode mudar a
-- cara de proposta já enviada. O padrão em ops_text_defaults é de onde
-- a proposta nova nasce preenchida.
--
-- Proposta sem foto continua saindo como antes, em creme. Não é
-- fallback de conveniência: é a saída certa para quem manda a proposta
-- sem imagem nenhuma.
--
-- Só toca em objetos ops_. Nada fora desse prefixo.
-- =====================================================================

alter table ops_proposals add column if not exists backcover_img text;
comment on column ops_proposals.backcover_img is
  'Foto de fundo da contracapa. Vazio: a contracapa sai em creme, com o logo terracota.';

insert into ops_text_defaults (key, pt, en) values
  ('backcover_img', 'https://i.postimg.cc/gcRDxpT8/Wedding-Booklet.png', null)
on conflict (key) do update
   set pt = coalesce(nullif(ops_text_defaults.pt,''), excluded.pt), updated_at = now();

-- As propostas apresentadas que já existem nascem com a foto da casa.
update ops_proposals
   set backcover_img = (select pt from ops_text_defaults where key='backcover_img'),
       updated_at = now()
 where layout = 'apresentada' and coalesce(backcover_img,'') = '';

-- tl_get_quote passa a devolver a foto. O objeto continua sendo montado
-- campo a campo: coluna nova não vaza por ter sido criada.
...
insert into ops_migrations (id) values ('0013-contracapa-com-foto') on conflict (id) do nothing;
